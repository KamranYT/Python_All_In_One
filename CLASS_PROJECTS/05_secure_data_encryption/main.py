...
menu = ["Home", "Register", "Login", "Stored Data", "Retrieve Data"]
...
if choice == "Home":
    ...
    if st.button("Login"):
        if username in stored_data and stored_data[username][
            "password"
    ...
    if not st.session_state.authenticated_user:
        st.warining("Please login first")
    else:
        st.subheader("Store Encrypted Data")
        data = st.text_area("Enter data to encrypt")
        passkey = st.text_input("Encryption key (passphrase)", type="password")

        if st.button("Envrypt and Save"):
            if data and passkey:
                ...
                save_data(stored_data)
                st.success("Data encrypted and sav sucessfully!")
    ...
    if not st.session_state.authenticated_user:
        ...
        user_data = stored_data.get(st.session_state.authenticated_user, {}).get(
            "data", []
        )
